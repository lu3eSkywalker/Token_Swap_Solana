...
describe("Test", () => {
  ...
  it("initializes a user liquidity account", async () => {

    const [userPDALiquidity, bump] = await web3.PublicKey.findProgramAddressSync(
      [Buffer.from("liquidityPDA"), program.provider.publicKey.toBuffer()],
      program.programId
    );
  ...
  it("adds liquidity to the liquidity pool", async () => {
    ...
    const [vaultTokenAAccount, bump] = await web3.PublicKey.findProgramAddressSync(
      [Buffer.from("vaultTokenA"), tokenA_mint_address.toBuffer()],
      program.programId
    );

    const [vaultTokenBAccount, bump2] = await web3.PublicKey.findProgramAddressSync(
      [Buffer.from("vaultTokenB"), tokenB_mint_address.toBuffer()],
      program.programId
    );

    const [userPDALiquidity, bump3] = await web3.PublicKey.findProgramAddressSync(
      [Buffer.from("liquidityPDA"), program.provider.publicKey.toBuffer()],
      program.programId
    );

    console.log("This is the vault token account for Token A: ", vaultTokenAAccount.toBase58());

    console.log("This is the vault token account for Token B: ", vaultTokenBAccount.toBase58());
    ...
    const txHash = await program.methods
      ...
      .accounts({
        ...
        vaultTokenBAccount: vaultTokenBAccount,
  ...
  it("removes liquidity from the liquidity pool", async () => {
    ...
    const [vault_auth_a, bump4] = await web3.PublicKey.findProgramAddressSync(
      [Buffer.from("vaultTokenA"), tokenA_mint_address.toBuffer()],
      program.programId
    );

    const [vault_auth_b, bump5] = await web3.PublicKey.findProgramAddressSync(
      [Buffer.from("vaultTokenB"), tokenB_mint_address.toBuffer()],
      program.programId
    );
